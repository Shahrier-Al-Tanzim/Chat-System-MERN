...
const Login: React.FC<{ onAuthedRoute?: string }> = ({ onAuthedRoute = '/chat' }) => {
  ...
  return (
    <div style={{ minHeight: '100vh', width: '100vw', display: 'flex', alignItems: 'center', justifyContent: 'center', background: 'radial-gradient(circle at 60% 40%, #b2f2bb 0%, #e6fcf5 100%)' }}>
      <div style={{ maxWidth: 420, width: '100%', padding: 40, borderRadius: 20, background: 'linear-gradient(135deg, #38d9a9 0%, #228B22 100%)', boxShadow: '0 8px 32px rgba(34,139,34,0.12)', display: 'flex', flexDirection: 'column', alignItems: 'center' }}>
        <h2 style={{ color: '#fff', textAlign: 'center', marginBottom: 20, fontSize: 32, fontWeight: 700, letterSpacing: 1 }}>Log in</h2>
        <form
          onSubmit={async (e) => {
            e.preventDefault();
            setErr(null);
            ...
            try {
              ...
              if (typeof e === 'object' && e !== null && 'response' in e) {
                ...
                setErr(errObj.response?.data?.message ?? 'Login failed');
              } else {
                setErr('Login failed');
              }
            } finally {
        ...
        >
          ...
          <button disabled={loading} style={{ background: '#fff', color: '#228B22', border: 'none', borderRadius: 10, padding: '12px 0', fontSize: 19, fontWeight: 700, width: '100%', cursor: loading ? 'not-allowed' : 'pointer', boxShadow: '0 2px 8px rgba(34,139,34,0.10)', marginTop: 6, transition: 'background 0.2s' }}>{loading ? 'Logging in...' : 'Log in'}</button>
        </form>
        <div style={{ marginTop: 22, textAlign: 'center' }}>
          <span style={{ color: '#fff', fontWeight: 500 }}>New here?</span> <Link to="/signup" style={{ color: '#b2f2bb', fontWeight: 700, textDecoration: 'underline' }}>Create an account</Link>
        </div>
      </div>
    </div>
  );
